...
const checkLocalStorage = () => {
  if (localStorage.getItem('wordle')) {
    ...
    if (savedData.day == day && answer == savedData.answer) {
...
const checktheRow = () => {
  if (endOfRow && canPlay) {
    ...
    // Check if we have entered an actual word
    const row = document.querySelectorAll('.board-row')[activeRow]
    const cell = row.querySelectorAll('.board-cell')
    let word = ""
    for (let i = 0; i < cell.length; i++) {
    ...
    // Loop through wordlist to look for word
    let found = 0
